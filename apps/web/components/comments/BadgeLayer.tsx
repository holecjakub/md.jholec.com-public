 ...
 * (rAF-batched); colliding badges nudge downward.
 *
 * Hover/focus on a badge raises the block id to the parent (onHoverBlock) so the
 * inline underline(s) for that block tint in — and the parent feeds `hoveredBlockId`
 * back so a hover starting on an underline lights the matching badge. Clicking a
...
export function BadgeLayer({
  ...
  const measure = useCallback(() => {
    ...
    const groups = groupThreadsByBlock(threads);
    const located: PlacedBadge[] = [];
    ...
    for (const group of groups) {
      ...
      if (!first) continue;
      const result = relocateAnchor(first.root.anchor, container);
      ...
      located.push({ group, top, blockBottom });
    }
    ...
    <>
      ...
      {placed.map((p) => (
        <PositionedBadge
          ...
          emphasized={hoveredBlockId === p.group.blockId}
          onOpen={onOpenBlock}
